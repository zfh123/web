...
<template>
  <div class="page-goods">
    <div class="row no-gutters p-2">
      <div class="col-3">
        <button type="button" class="btn btn-primary" @click.stop="$router.back()">返回</button>
      </div>
      <div class="col text-center">
        <h2 class="m-0"><span class="badge badge-danger">盘亏： ￥1234.0</span>
        </h2>
      </div>
      <div class="col-3 text-right">
        <div class="btn-group">
          <button type="button" class="btn"
                  ...
                  @click.prevent.stop="$router.replace('inventory')">全部</button>
          <button type="button" class="btn"
                  :class="{
                  'btn-dark':$route.query.type==='profit',
                  'btn-outline-dark':$route.query.type!=='profit'
                  }"
                  @click.prevent.stop="$router.replace({path:'inventory',query:{ type: 'profit' }})">盘盈</button>
          <button type="button" class="btn"
                  :class="{
                  'btn-dark':$route.query.type==='deficit',
                  'btn-outline-dark':$route.query.type!=='deficit'
                  }"
                  @click.prevent.stop="$router.replace({path:'inventory',query:{ type: 'deficit' }})">盘亏</button>
        </div>
      </div>
    </div>
    <hr class="m-0">
    <div class="table-warp2">
      <v-dataTable headHeight="33px" :data="items" v-model="active" :noDataMsg="noDataMsg" footHeight="50px">
        ...
        <template slot="thead">
          ...
          <th style="width: 10%" class="text-center">原库存</th>
          <th style="width: 10%" class="text-center">盘点库存</th>
          <th style="width: 30%">操作人</th>
        </template>
        <template slot="tbody" slot-scope="props">
          <td>某某某商品<span class="badge badge-danger">亏</span></td>
          <td class="text-center">10</td>
          <td class="text-center">10</td>
          <td><small class="text-muted">
            <span class="mr-1">朱芳芳</span>
            <span class="mr-1">朱芳芳2</span>
          </small></td>
        </template>
